...
export class KOLsService {
    ...
    private _kols: BehaviorSubject<any[]> = new BehaviorSubject([]);
    ...
    loadInitialData() {
        this._kols.next(this.data);
    }

    processFilters() {
        let isBookmarkSelected = this.filterService.getFilterByName('bookmark').selected;
        let isRisingStarSelected = this.filterService.getFilterByName('risingStar').selected;

        let activeDegreeFilters = this.filterService.getFiltersByCategory('degree');
        let hasActiveDegreeFilters = activeDegreeFilters.length > 0;
        ...
        let filteredData = this.data.filter(kol => {
            ...
            //Handle Bookmarks
            if (isBookmarkSelected && !kol.favorite) {
                return false;
            }

            //Handle Rising Stars
            if (isRisingStarSelected && kol.Score < 80) {
                return false;
            }

            //Handle Degree
            if (hasActiveDegreeFilters && activeDegreeFilters.indexOf(kol.Degree) == -1) {
                return false;
            }
